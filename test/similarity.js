...
for (let algorithm in distance) {
  ...
  result.allowZeroDiff = callback(v5, v9) !== NaN && callback(v5, v9) !== Infinity && callback(v5, v9) > 0;
  result.allowNegativeNumbers = callback(v10, v11) !== NaN && callback(v1, v11) !== Infinity && callback(v1, v11) > 0;
  result.scaleInsensitive = callback(v1, v3) === callback(v2, v4);
  results.push(result);
  console.log(callback(v10, v11));
}
